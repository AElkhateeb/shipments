...
class FillAgentAdminAndAdminPermissions extends Migration
{
    /**
    ...
    {
        $this->guardName = config('agent-auth.defaults.guard');
        ...
        $defaultPermissions = collect([
            ...
            // manage translations
            'agent.translation.index',
            'agent.translation.edit',
            'agent.translation.rescan',

            // manage users (access)

            'agent.manger-admin.index',
            'agent.manger-admin.create',
            'agent.manger-admin.edit',
            'agent.manger-admin.delete',
            'agent.manger-admin.impersonal-login',

            'agent.employee-admin.index',
            'agent.employee-admin.create',
            'agent.employee-admin.edit',
            'agent.employee-admin.delete',
            'agent.employee-admin.impersonal-login',

            'agent.shipper-admin.index',
            'agent.shipper-admin.create',
            'agent.shipper-admin.edit',
            'agent.shipper-admin.delete',
            'agent.shipper-admin.impersonal-login',

            'agent.account-admin.index',
            'agent.account-admin.create',
            'agent.account-admin.edit',
            'agent.account-admin.delete',
            'agent.account-admin.impersonal-login',
        ...
        $this->roles = [
            [
                ...
                'permissions' => $defaultPermissions->reject(function ($permission) {
                    return $permission === 'agent.agent-admin.impersonal-login';
                }),
            ],
        ];
        ...
        DB::transaction(function () {
            ...
            foreach ($this->users as $user) {
                ...
                if ($userItem === null) {
                    ...
                    AgentAdmin::find($userId)->addMedia(storage_path() . '/images/avatar.png')
                        ->preservingOriginal()
    ...
    public function down(): void
    {
        ...
        DB::transaction(function () {
            foreach ($this->users as $user) {
                ...
                if ($userItem !== null) {
                    AgentAdmin::find($userItem->id)->media()->delete();
