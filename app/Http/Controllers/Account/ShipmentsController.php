...
use App\Http\Requests\Account\Shipment\BulkDestroyShipment;
use App\Http\Requests\Account\Shipment\DestroyShipment;
use App\Http\Requests\Account\Shipment\IndexShipment;
use App\Http\Requests\Account\Shipment\StoreShipment;
use App\Http\Requests\Account\Shipment\UpdateShipment;
...
class ShipmentsController extends Controller
{
    ...
    public function index(IndexShipment $request)
    {
        ...
        return view('account.shipment.index', [
            'data' => $data,
            'roads' => Road::all(),
            'places' => Place::all(),
            'branches' => Branch::all(),
    ...
    {
        $this->authorize('account.shipment.create');

     
        return view('account.shipment.create', [
                'roads' => Road::all(),
                'places' => Place::all(),
                'branches' => Branch::all(),
                'paymentMethods' => PaymentMethod::all(),
                'roles' => Role::where('guard_name', '!=','account')->get(),
    ...
    public function store(StoreShipment $request)
    {
        ...
        if ($request->ajax()) {
            return ['redirect' => url('account/shipments'), 'message' => trans('brackets/admin-ui::admin.operation.succeeded')];
        }

        return redirect('account/shipments');
    }
    ...
    {
        $this->authorize('account.shipment.show', $shipment);
    ...
    {
        $this->authorize('account.shipment.edit', $shipment);


        return view('account.shipment.edit', [
            'shipment' => $shipment,
            'roads' => Road::all(),
            'places' => Place::all(),
            'branches' => Branch::all(),
    ...
    public function update(UpdateShipment $request, Shipment $shipment)
    {
        ...
        if ($request->ajax()) {
            return [
                'redirect' => url('account/shipments'),
